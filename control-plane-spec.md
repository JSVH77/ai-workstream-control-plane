...
- **`.claude/settings.json` is committed-baseline + generated-specialization.** Settings load at session **START**, so a purely *generated* (and gitignored) settings file leaves a hole: the session that clones the repo and runs `init.sh` has **no project floor at all** — no `rm -rf /`, no force-push deny, no `sudo` deny — and the generated file only takes effect at the *next* session. The repo therefore **commits a baseline** `.claude/settings.json` — the generic floor with `{{HOOK_NAMESPACE}}` resolved and **no** project-specific denies — so the safety floor is live from `git clone`. `gen-config` then **specializes** it per worktree (protected paths, extra denies, merge policy).

  Three consequences, all deliberate:
  1. **The baseline and the generated file differ**, so `.claude/settings.json` will normally show as *modified* in a worktree once `gen-config` has run. `doctor --compliance` compares the live file against **`gen-config` output** (not against the committed baseline), which is the correct oracle. After editing the floor, re-run `gen-config`.
  2. **Never commit a stream's specialized copy over the baseline** — the path is the same in every worktree, so doing so would push one stream's merge policy onto every other stream.
  3. **No layer can fix the *current* session.** Settings are read once, at start; `init.sh` therefore prints a restart notice. The bootstrap session is pre-floor by nature.

- **`.claude/settings.local.json`** is the personal, per-worktree override — gitignored, and **never** touched by `gen-config`. Machine-specific allows belong there, not in the generated `settings.json`.
...
- Tracked docs + templates: this spec, `README.md`, `QUICKSTART.md`, `DISASTER-RECOVERY.md`, `SA-BOOTSTRAP.md`, `charters/` (README + `_TEMPLATE.md` + generic `SA.template.md`/`CR.template.md`), `use-cases/UC-00..05`, `review-ledger.template.md`, `runtime.md`, `loops/`, `config/*.example.yaml`, `config/settings-base.json`, `init.sh`, and the portable memory starter set (`memory/` + `MEMORY.template.md`).

**Implemented — operator environment (per machine, NOT repo-auditable):** the worktrees themselves; the hooks installed under `~/.claude/hooks/<hook_namespace>/`; per-worktree `STATE.md` (gitignored), journals + transcript backups; each worktree's **specialized** `.claude/settings.json` (the *baseline* is tracked — §3.7 — but what `gen-config` writes on top of it is per-machine) and `settings.local.json`; the vendor memory bucket; and **this repo's own overlay** (`config/project.yaml`, `config/streams.yaml`, `ownership.yaml`, `review-ledger.md`, and the concrete `charters/*.md` — untracked here by design; see the README). These are real on the machine that has them but **by definition unverifiable from any other checkout**. To corroborate this paragraph where the worktrees live, run `wf doctor`. A reader elsewhere still cannot verify it — they would get *their* machine's topology.
...
- **Subtree round-trip** — consuming this repo via `git subtree --prefix=` works by construction (§1.3). The *upgrade* path for a host whose overlay has diverged is **narrowed but still unproven**: the concrete `charters/*.md` moved into the overlay set (they are gitignored here, templates ship instead), which removes the one *guaranteed* standing conflict — a host's `SA.md` carrying its own merge gates. What remains unproven is the genuinely-shared narrative core: a host that edits `README.md`, `control-plane-spec.md`, `QUICKSTART.md`, or `DISASTER-RECOVERY.md` in place still conflicts on every pull, and nothing yet forces those edits into an overlay. Not proven until an actual second pull lands on a diverged host (E3).
