...
`config/streams.yaml`, `ownership.yaml`, `charters/*`).
...
This repo tracks the framework **core** plus `*.example.yaml` templates. The concrete overlay —
`config/project.yaml`, `config/streams.yaml`, `ownership.yaml`, `review-ledger.md` — is **gitignored here**
and generated by `init.sh`.

That is deliberate: this repo is meant to be *consumed* (vendored via subtree), and an upstream pull must
never clobber a host's own registry or ledger. In a **host** project those same files *are* tracked (Tier A)
— that is where they belong. The trade: a fresh clone of *this* repo can't run `decoupling-lint` or
...
charters/           README · _TEMPLATE.md · SA.md          (apply-authority; `governs:` points here)
